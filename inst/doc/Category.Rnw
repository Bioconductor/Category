...
The mean plot for the category with the smallest aggregate statistic.}
\end{figure}
...
The heatmap for the category with the Ribosome pathway.}
\end{figure}
...
\begin{figure}[tp]
  \centering
<<mnplotX, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
@
\includegraphics[width=1.25\smallfigwidth]{Category-mnplotX}
