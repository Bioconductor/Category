...
First we subse the ALL data to the two phenotypes that we would like
...
fL = findLargest(geneNames(esetASub), abs(ttests$statistic), "hgu95av2")
...
<<reorder, echo=FALSE, results=hide>>=
eS = esetASub[match(names(fL2), geneNames(esetASub)),]
...
\begin{figure}[tp]
  \centering
<<qqplot, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
 qqnorm(tA)
@
...
\begin{figure}[tp]
  \centering
<<mnplot, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
    KEGGmnplot(byTT, eS, group=eS$mol, data="hgu95av2",
               main=paste(KEGGPATHID2NAME[[byTT]], paste("Overall:",
               round(tA[byTT], 3)), sep="\n"))

@
\includegraphics[width=1.25\smallfigwidth]{Category-mnplot}
\caption{\label{fig:mn}%
The mean plot for the category with the smallest p-value.}
\end{figure}

Figure~\ref{fig:mn} suggests that the level of activity among genes
...
selected by our filtering approach, Figure~\ref{fig:hmRib1}, and then
we examine the heatmap for all genes that are on the chip and
annotated at the ribosome pathway, Figure~\ref{fig:hmRib2}.
...
#hmout = heatmap(pAC, col=hmcol, ColSideColors=spcol, keep.dendro=TRUE)
@
...
\begin{figure}[tp]
  \centering
<<mnplot, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
    tmp2 = KEGG2heatmap(byTT, esetA, data="hgu95av2",
               main=paste(KEGGPATHID2NAME[[byTT]], paste("Overall:",
               round(tA[byTT], 3)), sep="\n"), col=hmcol,
                 ColSideColors=spcol)
...
biological, but is also uninteresting. Gender is slightly confounded
with the relationship between BCR/ABL and NEG cytogenetics and hence
we will sometimes find effects that may be more correctly attributed
to sex.
...
testing, and so is will not be too dramatic.

We can visualize the differences in group means, just as we did
before. These are shown in Figures~\ref{fig:mn1} through \ref{fig:mn3}.

\begin{figure}[tp]
  \centering
<<mnplot1, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
The per category mnplot pathway that are deemed to be
differentially expressed using a permutation approach.}
\end{figure}
...
<<sanityCheck, echo=FALSE, results=hide>>=
stopifnot(pwtt$statistic[mostI] > 4.5)
stopifnot(whpw == "04610")

@

We see that the category which has the largest $t$-test statistic is
\Sexpr{ KEGGPATHID2NAME[[whpw]]} and could explore this further.
